...
{
    public class AccountBulkEntryController : Controller
    {
        //
        // GET: /AccountBulkEntry/
        ...
        public ActionResult FileUpload(FormCollection frm)
        {
            ...
            if (file.ContentLength > 0)
            {
                ...
                for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                {
                    ...
                    using (DBEntities db = new DBEntities())
                    {
                        db.SET_CLIENTACCOUNT_CLIENTACCOUNTDOCS_INSERT(CLIENT_NO, ds.Tables[0].Rows[i][0].ToString(),
                           Convert.ToDateTime(ds.Tables[0].Rows[i][1].ToString()), ds.Tables[0].Rows[i][2].ToString(),
                            ds.Tables[0].Rows[i][3].ToString(), ds.Tables[0].Rows[i][4].ToString(),
                            ds.Tables[0].Rows[i][5].ToString(), ds.Tables[0].Rows[i][6].ToString(),
                            ds.Tables[0].Rows[i][7].ToString(), ds.Tables[0].Rows[i][8].ToString(),
                            ds.Tables[0].Rows[i][9].ToString(), null, null);
                    }
                }
